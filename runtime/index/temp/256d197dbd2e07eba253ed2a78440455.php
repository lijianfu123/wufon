...
<div class="swiper-container swiper-container1">
    <div class="swiper-wrapper">
        <?php $Model = new app\common\model\SwiperModel();$detail = $Model->get_pass_detail(6);$__list__ = $detail["image_list_json"];if(is_array($__list__) || $__list__ instanceof \think\Collection || $__list__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__list__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?>
...
<!--一级分类-->
<ul class="switchTab">
    <?php $Model = new app\common\model\DownloadClassModel();$__list__ = $Model->where("parent_id = 0")->select();if(is_array($__list__) || $__list__ instanceof \think\Collection || $__list__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__list__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;$list_url = "download_list_url";$value[$list_url] = download_list_url($value["id"]); ?>
        <li class="rotateInDownRight animated wow <?php if(($value['id'] == $main_detail['id']) || ($value['id'] == $main_detail['parent_id'])): ?>aboutPlateCheck<?php endif; ?>">
            <a href="<?php echo htmlentities($value['download_list_url']); ?>"><?php echo htmlentities($value['download_class_name']); ?></a>
        </li>
    <?php endforeach; endif; else: echo "" ;endif; ?>
</ul>

<!--二级分类
<?php $download_class_id = $main_detail['level'] == 2 ? $main_detail['parent_id']: $main_detail['id']; ?>
<ul class="wrap classifyList">
    <?php $Model = new app\common\model\DownloadClassModel();$__list__ = $Model->where("parent_id = $download_class_id")->select();if(is_array($__list__) || $__list__ instanceof \think\Collection || $__list__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__list__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;$list_url = "download_list_url";$value[$list_url] = download_list_url($value["id"]); ?>
    <li>
        <a href="<?php echo htmlentities($value['download_list_url']); ?>"><?php echo htmlentities($value['download_class_name']); ?></a>
    </li>
    <?php endforeach; endif; else: echo "" ;endif; ?>
...
<!--        <td style="width: 100px;">大小</td>-->
    ...
    <tbody>
    <?php if(is_array($main_page['list']) || $main_page['list'] instanceof \think\Collection || $main_page['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $main_page['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?>
        <tr>
            <td style="text-align: left; padding-left: 10px;">
<!--                <a href="<?php echo htmlentities($value['download_detail_url']); ?>"><?php echo htmlentities($value['download_name']); ?></a>-->
                <a href="<?php echo url('download?id=' . $value['id']); ?>"><?php echo htmlentities($value['download_name']); ?></a>
            </td>
<!--            <td><?php echo htmlentities($value['file_size']); ?>M</td>-->
            <td><?php echo htmlentities($value['download_count']); ?></td>
            <td>
                <!--
                <a href="<?php echo url('download?id=' . $value['id']); ?>" target="_blank">
                <?php if((substr($value['file_url'], -3) == 'rar') || (substr($value['file_url'], -3) == 'zip')): ?>下载<?php else: ?>查看<?php endif; ?>
                </a>
                -->
                <?php if($value['file_url'] == ''): ?>
                    <a href="<?php echo htmlentities($value['download_detail_url']); ?>">查看</a>
                <?php else: ?>
                    <a href="<?php echo url('download?id=' . $value['id']); ?>" target="_blank">下载</a>
                <?php endif; ?>
            </td>
        </tr>
    <?php endforeach; endif; else: echo "" ;endif; ?>

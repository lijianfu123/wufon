...
<div class="swiper-container swiper-container1">
    <div class="swiper-wrapper">
        <?php $Model = new app\common\model\SwiperModel();$detail = $Model->get_pass_detail(3);$__list__ = $detail["image_list_json"];if(is_array($__list__) || $__list__ instanceof \think\Collection || $__list__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__list__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?>
...
<!--二级分类-->
<?php $product_class_id = $main_detail['level'] == 2 ? $main_detail['parent_id']: $main_detail['id']; ?>
<ul class="wrap classifyList">
    <?php $Model = new app\common\model\ProductClassModel();$__list__ = $Model->where("parent_id = $product_class_id")->select();if(is_array($__list__) || $__list__ instanceof \think\Collection || $__list__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__list__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;$list_url = "product_list_url";$value[$list_url] = product_list_url($value["id"]); ?>
        <li>
            <a href="<?php echo htmlentities($value['product_list_url']); ?>"><?php echo htmlentities($value['product_class_name']); ?></a>
        </li>
    <?php endforeach; endif; else: echo "" ;endif; ?>
...
<ul class="wrap navigation" style="margin: 10px auto;">
    ...
    <li class="rotateInDownLeft animated wow">
        <span class="selectText"><?php echo htmlentities($main_detail['product_class_name']); ?></span>
    </li>
</ul>
...
<div class="product-wrap wrap">
    <ul>
        <?php if(is_array($main_page['list']) || $main_page['list'] instanceof \think\Collection || $main_page['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $main_page['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?>
            <li>
                <a href="<?php echo htmlentities($value['product_detail_url']); ?>">
                    <img src="<?php echo htmlentities($value['image']); ?>" alt="产品"/>
                    <p class="product-info"><?php echo htmlentities($value['product_name']); ?></p>
                </a>
            </li>
        <?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
